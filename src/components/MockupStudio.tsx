...
/** Base URL of the mockup API. Empty = same origin (Lovable app).
 *  For a static host (Hostinger) set window.__API_BASE__ in index.html. */
function apiBase(): string {
  const w = window as unknown as { __API_BASE__?: string };
  const fromWindow = (w.__API_BASE__ ?? "").trim();
...
function base64ToBlob(dataUrl: string, type = "image/png"): Blob {
...
export default function MockupStudio() {
  ...
  async function downloadAll() {
    ...
    try {
      ...
      saveBlob(blob, "mockups.zip");
    } catch {
      // Fallback: save images one by one if zipping is blocked
      ready.forEach((r, i) => {
        setTimeout(
          () => saveBlob(base64ToBlob(r.src as string), `${String(i + 1).padStart(2, "0")}-${r.id}.png`),
          i * 400,
        );
      });
    } finally {
  ...
  async function generateOne(source: File, index: number) {
    ...
    if (!scene) return;
    setResults((r) =>
      r.map((x, i) => (i === index ? { ...x, status: "loading", error: undefined } : x)),
    );
    try {
      const fd = new FormData();
      fd.append("image", source);
      fd.append("prompt", scene.prompt);
      const res = await fetch(`${apiBase()}/api/mockup`, { method: "POST", body: fd });
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    {r.src ? (
                      <button
                        ...
                        onClick={() =>
                          saveBlob(
                            base64ToBlob(r.src as string),
                            `${String(i + 1).padStart(2, "0")}-${r.id}.png`,
                          )
                        }
